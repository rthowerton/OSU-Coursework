	...
	while(1)
	{
	   ...
	   switch(spawnpid)
	   {
		 ...
		 if(buffer[0] != 'd'){
		    ...
		    error("SERVER: Not connected to otp_dec.\n");
		 }
		 else
		    charsRead = send(establishedConnectionFD, buffer, strlen(buffer), 0);
		 ...
		 memset(temp, '\0', sizeof(buffer) - kspan);
	         ...
	         //check if key is as long as the plaintext
		 if(strlen(key) < strlen(buffer)) error("ENCRYPTION ERROR: key is shorter than plaintext\n");
	         //do ciphertext
