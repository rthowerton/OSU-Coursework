...
int main(int argc, char *argv[])
{
	...
	//fgets(buffer, sizeof(buffer) - 1, ciphertext); // Get ciphertext, place in buffer
	fgets(cipher, sizeof(cipher) - 1, ciphertext); // Get ciphertext, place in cipher
	cipher[strcspn(cipher, "\n")] = 'k'; //signal that ciphertext is complete with a k character

	//unpack key
	fgets(key, sizeof(key) - 1, keytext);
